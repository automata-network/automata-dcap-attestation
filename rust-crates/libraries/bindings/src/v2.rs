//! Compact V2 bindings. Legacy generated modules are deliberately retained.
alloy::sol! {
    #[sol(rpc)]
    interface IAutomataDcapAttestationV2 {
        function verifyAndAttestOnChainV2(bytes calldata quote, uint32 tcbEval, bool minCheck)
            external payable returns (bool success, bytes output, bytes quoteBody);
        ...
        function programModeV2(uint8 backend, bytes32 identifier) external view returns (bool registered, bool minCheck);
        event AttestationSubmittedV2(bool success, uint8 verifierType, uint16 indexed formatMajorVersion, uint16 indexed formatMinorVersion, bytes32 programIdentifier, bool minCheck, bytes output);
    }
    #[sol(rpc)]
    interface IAutomataDcapAttestationV2Default {
        function verifyAndAttestOnChainV2(bytes calldata quote) external payable returns (bool success, bytes output, bytes quoteBody);
